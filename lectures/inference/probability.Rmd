...
There are 42 people in this room. What is the chance that at least two people have the same birthday?

We can use a Monte Carlo simulation.

Assessment: Use `sample` to randomly select 42 birthdays. Check if they are unique.

```{r}
bdays = sample(1:365,42,replace=TRUE)
length(unique(bdays))
duplicated (bdays)
any(duplicated(bdays))
sum(duplicated(bdays))
table(table(bdays))
p42 = replicate(10000,any(duplicated(sample(1:365,42,replace=TRUE))))
mean(p42)
```
