...
```{r}
tab %>% group_by(week) %>% summarize(num_polls=n()) %>% filter(num_polls>=5)
```
...
```{r}
tab %>% filter(start_date>"2008-01-01" & week <=-30) %>% group_by(week) %>% mutate(num_polls=n()) %>% filter(num_polls>=5) %>% ungroup() %>% summarize(theta_hat=mean(diff),sigma_hat=sd(diff)) 
